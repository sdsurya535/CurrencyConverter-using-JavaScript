...
for (let select of dropdowns) {
  for (code in countryList) {
    ...
    createOpt.innerText = code;
    createOpt.value = code;
...
const updateFlag = (elm) => {
  ...
  image = elm.parentElement.querySelector("img");
  image.src = newSrc;
};

async function qualityExchange() {
  ...
  let newRate = Math.floor(amountVal * rate * 100) / 100;

  let newMsg = `${amountVal} ${fromVal.value} = ${newRate} ${toVal.value}`;
